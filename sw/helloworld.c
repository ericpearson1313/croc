...
// ASCON test 
////////////////////////////////
// Test data from ascon-verilog test.py:14 changed to RUNS = (9,10) and run make sims
// todo: Add NIST 800-232 defined tests

// Fixed read only data, no change
...
char msg2[ 9] = { 0xFC, 0xCA, 0x19, 0x6B, 0x3B, 0xB3, 0x09, 0x8C, 0xEE }; // for cxof input
...
// Initial pt, ct data will be in-place overwritten but we have OG versions above for comparison
char   pt[9]  = { 0x32, 0x3B, 0x41, 0xEE, 0x00, 0xAE, 0x8A, 0x14, 0xAA }; 
char   ct[9]  = { 0x3C, 0x71, 0xC7, 0xBA, 0xDE, 0x48, 0x01, 0x2E, 0x1D }; 
// Output buffers written by cipher unit
char  tag[16];
char auth[ 4];
char fail[ 4];
char hash[32];
...
int main() {
    ...
    /////////////////////////////////////////
    // Build & Run Cipher Test Command List
    /////////////////////////////////////////
    printf("ASCON test\n");
    int  ad_len   = 9;
    int  msg_len  = 9;
    ...
    // Use a single command list to run the 5 cipher operations on the test data
    long cmd[35];
    int  cidx = 0;
    ...
    cmd[cidx++] = (long)npub;
    cmd[cidx++] = (long)ad;
    cmd[cidx++] = (long)ct; // r (no write if auth fails, any write will cause next test to fail)
    cmd[cidx++] = (long)key; // != tag, will fail
    cmd[cidx++] = (long)fail; // w
    cmd[cidx++] = (2/*DEC*/<<28) + (ad_len<<12) + (msg_len<<0); // { cmd[7:0], ad_len[11:0], msg_len[11:0] }
    cmd[cidx++] = (long)key;
    cmd[cidx++] = (long)npub;
    cmd[cidx++] = (long)ad;
    ...
    cmd[cidx++] = hash_len;
    cmd[cidx++] = (long)ctsm; // read this as context and msg len
    ...
    //////////////////////////////////////
    //
    long done = 0x2011111; // status flags indicating cipher and dma units are complete
    hw_reg[4] = cidx<<2;   // command list length (as bytes
    hw_reg[1] = (long)cmd; // Start execution on command list, write command list addr to the HW register
    while( (hw_reg[6] & done) != done ); // poll for completion
    //
    //////////////////////////////////////

    printf("Done\n");
    printf("Auth %x Fail %x [", *(long *)auth, (long *)fail );
	...
	// fail should be "fail"
        for( int ii = 0; ii < 4; ii++ ) 
		if( fail[ii] != ((ii == 0)?'f':(ii==1)?'a':(ii==2)?'i':'l')) { err++; }
	// pt should = oct
